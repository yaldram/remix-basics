...
export default function FetcherLoadingStatesPage() {
  ...
  }, [addFetcher.data?.status, deleteFetcher.data?.status]);

  const addDisabled = addFetcher.state !== "idle"
  const deleteDisabled = deleteFetcher.state !== "idle"
          ...
          <Card key={repository.id} className="w-[500px]  divide-y divide-solid">
            ...
            <CardFooter className="flex gap-4 p-6">
              <deleteFetcher.Form method="POST" className="w-full">
                ...
                <Button disabled={deleteDisabled} type="submit" className="w-full" variant="destructive">
